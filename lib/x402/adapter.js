...
export async function verifyPayment(paymentPayload, requirements, config) {
  const provider = getProvider(config.x402Mode);
  // Pass config for coinbase provider (needs facilitatorUrl, network)
  // Mock provider doesn't use config but accepts it for compatibility
  return provider.verifyPayment(paymentPayload, requirements, config);
}
...
export async function settlePayment(paymentPayload, requirements, config) {
  ...
  // Pass config for coinbase provider (needs facilitatorUrl, network)
  // Mock provider doesn't use config but accepts it for compatibility
  return provider.settlePayment(paymentPayload, requirements, config);
}
