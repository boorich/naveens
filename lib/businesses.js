...
export function list() {
  ...
  return database.prepare(`
    SELECT slug, driver_name AS name, service_name AS serviceName,
           ...
           manage_token AS manageToken, created_at AS createdAt
...
export function listProducts(slug) {
  const database = getDb();
  if (!database) return [];
  return database.prepare(`
    SELECT id, slug, name, lkr_price AS lkrPrice, sort_order AS sortOrder
    FROM products WHERE slug = ? ORDER BY sort_order ASC, id ASC
  `).all(slug);
}

export function createProduct(slug, name, lkrPrice) {
  const database = getDb();
  if (!database) throw new Error('Database not configured');
  const result = database.prepare(`
    INSERT INTO products (slug, name, lkr_price) VALUES (?, ?, ?)
  `).run(slug, name, lkrPrice);
  return database.prepare(`
    SELECT id, slug, name, lkr_price AS lkrPrice, sort_order AS sortOrder
...
export function deleteProduct(id, slug) {
  const database = getDb();
  if (!database) return;
  database.prepare(`DELETE FROM products WHERE id = ? AND slug = ?`).run(id, slug);
}
